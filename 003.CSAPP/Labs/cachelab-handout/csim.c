...
void update(unsigned int address)
{
    ...
    int setIndex = (address >> b) & ((-1U) >> (64 - s));
    ...
    {
        if (_cache_[setIndex][i].tag == tag_add)
        {
            _cache_[setIndex][i].time_stamp = 0;
            ++hit_count;
            return;
        }
    }
    ...
    {
        if (_cache_[setIndex][i].valid_bits == 0)
        {
            _cache_[setIndex][i].valid_bits = 1;
            _cache_[setIndex][i].tag = tag_add;
            _cache_[setIndex][i].time_stamp = 0;
            ++miss_count;
            return;
        }
    ...
    for (int i = 0; i < E; ++i)
    {
        if (_cache_[setIndex][i].time_stamp > max_stamp)
        {
            max_stamp = _cache_[setIndex][i].time_stamp;
            max_stamp_index = i;
        }
    }
    _cache_[setIndex][max_stamp_index].tag = tag_add;
    _cache_[setIndex][max_stamp_index].time_stamp = 0;
    return;
}
